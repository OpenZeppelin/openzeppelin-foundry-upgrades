...
If you are deploying upgradeable contracts, use the `Upgrades` library as described in xref:foundry-upgrades#installing[Using with Foundry - Usage] but set the option `defender.useDefenderDeploy = true` when calling functions to cause all deployments to occur through OpenZeppelin Defender.
